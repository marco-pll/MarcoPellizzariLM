...
# ECCEZIONI #
Le procedure descritte in precedenza per lanciare le simulazioni sono sufficienti nella maggior parte dei casi. In alcuni casi la procedura da seguire può differire, e ripercorrere passo-passo quanto fatto nel lavoro è l'unico modo per riprodurre esattamente i risultati. Tutto questo è piuttosto tedioso: al momento della scrittura del codice non ero in possesso dell'abilità richiesta per organizzare meglio il codice.

### Simulazioni per GB - Quantità di alberi ridotta
In alcune simulazioni è stato stimato il solo gradient boosting, con un numero di alberi massimo molto ridotto. Lanciare l'intero script "SLURM_SimulationsLight.R" non ha senso, in quanto verrebbero stimati nuovamente le foreste casuali e i ridge. Quindi, in quei casi è stato lanciato lo script "SLURM_SimulationsLightGB.R", identico nel funzionamento, ma le parti relative agli altri modelli sono state tagliate.
...
### Stagionalità e processi autocorrelati
...
In modo analogo, la stessa cosa va fatta per le simulazioni che contengono processi autocorrelati. La variabile quant dovrà quindi contenere, oltre alle X (["V1",...,"V10"]) anche i primi ritardi della media giornaliera. Basta guardare i dataset prodotti dalle funzioni per determinare i nomi delle variabili da includere.
